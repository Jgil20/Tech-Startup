...
// Get post_id from URL or default
...
// Get post_id from URL or fallback to default
$post_id = isset($_GET['id']) ? intval($_GET['id']) : 1;

// Handle form submission
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $submitted_post_id = isset($_POST['post_id']) ? intval($_POST['post_id']) : 1;
    $name = $conn->real_escape_string($_POST['name']);
    $email = $conn->real_escape_string($_POST['email']);
    $subject = $conn->real_escape_string($_POST['subject']);
    $message = $conn->real_escape_string($_POST['message']);

    $sql = "INSERT INTO comments (post_id, name, email, subject, message) VALUES ('$submitted_post_id', '$name', '$email', '$subject', '$message')";

    if ($conn->query($sql) === TRUE) {
        echo "<p class='alert alert-success'>New comment added successfully!</p>";
    } else {
...
<div class="row">
    <div class="col-lg-9">
        <div class="comments-area sec-mar">
            <h3><?php echo $result ? $result->num_rows : 0; ?> Comment(s)</h3>
